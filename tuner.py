...
PARAM_MODS = {
    ...
    "rfp_margin_base": (-9, -1000, 4000, 75),
    "rfp_margin_factor": (699, 0, 4000, 75),

    "afp_margin": (28826, 10000, 50000, 3000),

    "null_move_r_base": (4.47, 0.0, 8.0, 0.2),
    "null_move_r_factor": (0.126911611, 0.0, 0.5, 1./30),
    "null_move_r_denom": (2458, 1000, 15000, 200),

    "razoring_margin": (2569, 1000, 5000, 100),

    "lmr_base": (0.585, 0.0, 2.0, 0.2),
    "lmr_factor": (0.5036, 1./4, 1.0, 0.1),
    "lmr_history_denominator": (7780, 2000, 16000, 400),

    # "lmr_cut_base": (0.585, 0.0, 2.0, 0.35),
    # "lmr_cut_factor": (0.5036, 1./4, 1.0, 0.07),

    "lmp_improving_base": (4.23555, 0.0, 10.0, 0.3),
    "lmp_improving_factor": (1.02456, 0.1, 2.0, 0.1),
    "lmp_nonimproving_base": (2.126, 0.0, 10.0, 0.3),
    "lmp_nonimproving_factor": (0.54349, 0.1, 2.0, 0.1),

    "history_decay_factor": (0.0018623, 1.0 / 1024.0, 1.0 / 100.0, 0.0005),
    "history_delta_factor": (31.952, 10.0, 50.0, 2.0),

    "history_leaf_pruning_margin": (5444, 0, 30000, 500),
    "countermove_pruning_factor": (-799, -4000, 0, 100),
    "followup_pruning_factor": (-1753, -5000, 0, 175),

    "singular_margin_factor": (35, 10, 100, 5),

    # "delta_margin_factor": (2, 0.5, 4.0, 0.2),
    # "delta_margin_base": (1000, 0, 5000, 250)
}
...
def normalize_params(params):
    ...
    for k in params:
        ...
        if k in INTEGER_PARAMS:
            ...
            new_params[k] = min(int(hi), max(int(lo), int(param)))
        else:
...
def run_verification_games():
    ...
    error = tail[end:(end + tail[end:].find('\n'))]
    print("ERROR: ", error)
