...
{
  // This model was written by Kade <helo@expositor.dev>. It has no theoretical
  //   basis – it was empirically derived by attempting to fit game data.

  let x = ply as f64;
  let m = material_delta.abs() as usize;

  // When the material difference m is small, we model the expected
  //   number of remaining ply as a piecewise linear function.

  if m < 4 {
    let bk1 = [ 32.0,  31.0,  34.0,  51.0]; // breakpoint 1
    let bk2 = [114.0, 124.0, 121.0, 124.0]; // breakpoint 2
    ...
    return if x >= bk2[m] {  c[m] }               // constant regime
      else if x >= bk1[m] { a2[m] - x * b2[m] }   // second linear regime
      else                { a1[m] - x * b1[m] };  // first linear regime
  }

  // When the material difference m is large, we model the expected number
  //   of remaining ply as linear function with an exponential tail.

  let m = cmp::min(m, 19) - 4;
  ...
  // exponential regime
  if x >= bk[m] { return c[m] + d[m] * (-x / p[m]).exp(); }
...
pub fn get_time_bounds_clock_inc(clock: i32, inc: i32, overhead: i32, ply: i32, material: i32) -> (u128, u128) {
    ...
    let max_time_factor = 1.5;
    let maximum_time = cmp::min((est_time_per_move * max_time_factor).ceil() as u128, cmp::max(0, clock - overhead) as u128);
